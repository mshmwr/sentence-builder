...
- **每日挑戰紀錄日曆**（「歷史」畫面頂部）：僅登入時追蹤，GitHub 貢獻圖式的月曆熱力圖。
  `users/{uid}.practiceDays`（`{"YYYY-MM-DD": 當天過關次數}`）每次過關都累加，
  不受歷史紀錄 50 筆上限影響。格數依當天次數分三級深淺（達到每日目標 3 句最深），
  今天的格子有金框；可翻月，不能翻到未來。**點一天的格子**會在下方顯示明確數字
  （「YYYY-MM-DD：拼了 N 句」，不用再靠 hover tooltip），同時把下面的歷史清單篩選成
  只顯示那天拼的句子；若那天的次數來自 `practiceDays`（不會過期）但超出歷史紀錄
  50 筆上限，會改顯示提示文字說明明細看不到了，而不是誤導成「這天沒拼過」。
- **字級縮放（A-/A+）**：帳號列左邊的 A-/A+，5 級（0.85x〜1.45x），存 localStorage
  （`pinju-font-scale`，跟裝置綁定，不是帳號資料，不登入也能調）。用 `document.body.style.zoom`
  而不是改根字級，因為畫面的尺寸大多是寫死的 px，只放大文字沒用，要連版面一起放大字才有意義。
  另外全域加了 `text-size-adjust: 100%`，讓手機本身「字體大小」這個無障礙設定不要再繞過這裡的
  版面設計去硬放大文字——這正是這個功能的起因：使用者的手機設定字體放大後，畫面版面跑掉、不好練習。
